...
# -------------------------------------------------------
# 1. Pega o diretório onde ESTE arquivo (asset_loader.py) está: .../wumpus-world/gui
GUI_DIR = os.path.dirname(os.path.abspath(__file__))

# 2. Sobe um nível para chegar na raiz do projeto: .../wumpus-world/
ROOT_DIR = os.path.dirname(GUI_DIR)

# 3. Define onde estão as pastas de imagens e audios
...
#  TAMANHO DAS CÉLULAS
# -------------------------------------------------------
# Dica: Idealmente, constantes numéricas devem ir para utils/constants.py no futuro
CELL_SIZE = 120
...
def load_image(key, width=CELL_SIZE, height=CELL_SIZE):
    ...
    if not path or not os.path.exists(path):
        ...
        # Retorna uma superfície rosa choque para indicar erro visualmente (técnica comum em gamedev)
...
def load_all_assets():
    """
    Retorna um dicionário com todas as imagens já carregadas e processadas pelo Pygame.
    """
    assets = {}

    # Carrega elementos estáticos
    assets["GROUND"] = load_image("GROUND")
    assets["PIT"] = load_image("PIT")
    assets["GOLD"] = load_image("GOLD")
    assets["WUMPUS"] = load_image("WUMPUS")

    # Se você tiver a imagem de nevoa/unexplored
    if "UNEXPLORED" in ASSET_PATHS:
        assets["UNEXPLORED"] = load_image("UNEXPLORED")

    # Carrega as 4 posições do agente
    ...
    # Define uma imagem padrão para o agente (começa virado para cima)
